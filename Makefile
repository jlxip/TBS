...
# Some of these variables are binary (yes/no) and some have text.
# ISLIB := yes
# CXXEX := cc
# Binary variables that you want to set to true may be set to any value.
# Example: ISLIB := yeah
# Binary variables that you want to set to false may be unset.
# Variables are unset (overriding thus the default behavior) by setting
# them to an empty value. Example: CEX :=
...
# This is enough for most simple projects. Try it, see what went wrong,
# and then continue reading to fix it.
...
# $(RESULTLINK) to the desired resulting filename, for instance:
...
# together unless otherwise specified. The language of a file is known by
# its extension:
#   C   -> $(CEX), defaults to ".c"
...
#   - Fixed a bug regarding $(EXCLUDE)
# - 1.0.0-RC3 (2023-03-04):
#   - Fixed bug with STRIP macro definition
